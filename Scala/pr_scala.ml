...
module Id = struct
  value name = "Camlp4.Printers.Scala";
...
module Make (Syntax : Sig.Camlp4Syntax) = struct
  ...
  object (o)
    ...
    | <:ident< $i1$ $i2$ >> -> pp f "%a@,(%a)" o#type_ident i1 o#type_ident i2
    | <:ident< $anti:s$ >> -> o#anti f s
    | <:ident< $lid:s$ >> | <:ident< $uid:s$ >> -> o#type_var f s ];
    ...
    | <:patt< $str:s$ >> -> pp f "\"%s\"" s
    | <:patt< ( $p$ : $t$ ) >> -> pp f "@[<1>%a :@ %a@]" o#patt p o#ctyp t (* HERE *)
    ...
    | Ast.TyDcl _ tn tp te cl -> do {
        pp f "@[<2>%a%a@]" o#type_params tp o#type_var tn;
        match te with
        [ <:ctyp<>> -> ()
      ...
      | <:str_item< module $s1$ ($s2$ : $mt1$) = $me$ >> ->
          ...
          [ (al, me, Some mt2) ->
              pp f "@[<2>module %a@ @[<0>%a@] :@ %a =@ %a%(%)@]"
                o#type_var s1 o#functor_args al o#module_type mt2
                o#module_expr me semisep
          | (al, me, _) ->
              pp f "@[<2>module %a@ @[<0>%a@] =@ %a%(%)@]"
                o#type_var s1 o#functor_args al o#module_expr me semisep ]
      | <:str_item< module $s$ : $mt$ = $me$ >> ->
            pp f "@[<2>module %a :@ %a =@ %a%(%)@]"
              o#type_var s o#module_type mt o#module_expr me semisep
      | <:str_item< module $s$ = $me$ >> ->
            pp f "@[<2>module %a =@ %a%(%)@]" o#type_var s o#module_expr me semisep
      | <:str_item< module type $s$ = $mt$ >> ->
            pp f "@[<2>module type %a =@ %a%(%)@]"
              o#type_var s o#module_type mt semisep
      | <:str_item< open $sl$ >> ->
    ...
    method scala_params (f: string) (tp: string): string =
    ...
    | <:module_type< functor ( $s$ : $mt1$ ) -> $mt2$ >> ->
          pp f "@[<2>functor@ @[<1>(%a :@ %a)@]@ ->@ %a@]"
            o#type_var s o#module_type mt1 o#module_type mt2
    | <:module_type< '$s$ >> -> pp f "'%a" o#type_var s
    ...
    | <:module_expr< functor ( $s$ : $mt$ ) -> $me$ >> ->
          pp f "@[<2>functor@ @[<1>(%a :@ %a)@]@ ->@ %a@]" o#type_var s o#module_type mt o#module_expr me
